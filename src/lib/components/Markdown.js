...
export const Markdown = (props) => {
  const { onLinkClick, text, onMention, onHashtag, syntaxHighlighterProps } = props;
  return (
    <ReactMarkdown
      plugins={[]}
      rehypePlugins={[]}
      ...
      components={{
        ...
        a: ({ node, ...props }) =>
          onLinkClick ? <a onClick={onLinkClick} {...props} /> : <a target="_blank" {...props} />,
        img: ({ node, ...props }) => <img className="img-fluid" {...props} />,
        blockquote: ({ node, ...props }) => <blockquote className="blockquote" {...props} />,
        table: ({ node, ...props }) => <table className="table table-striped" {...props} />,
        code({ node, inline, className, children, ...props }) {
          const match = /language-(\w+)/.exec(className || '');
          const { wrapLines, lineProps, showLineNumbers, lineNumberStyle } = syntaxHighlighterProps ?? {};
