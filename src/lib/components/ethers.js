...
export const Web3ConnectButton = (props) => {
  ...
  const [{ wallet, connecting }, connect, disconnect] = ethersProviderContext?.useConnectWallet
    ? ethersProviderContext?.useConnectWallet()
    : [{}];

  return (
    <button
      className={`btn ${props.className} ${connecting || wallet ? 'btn-outline-secondary' : 'btn-outline-primary'}`}
